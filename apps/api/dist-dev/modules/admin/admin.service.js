...
const permissions_catalog_1 = require("../../common/permissions-catalog");
/**
...
let AdminService = class AdminService {
    constructor(prisma, vault, health, rail, providers) {
        this.prisma = prisma;
        this.vault = vault;
        this.health = health;
        this.rail = rail;
    ...
    async createRail(dto) {
        if (!['NSE_EIPO', 'BSE_IBBS'].includes(dto.exchange))
            throw new common_1.BadRequestException('Invalid exchange');
        if (!['live', 'uat'].includes(dto.env))
            throw new common_1.BadRequestException("env must be 'live' or 'uat'");
    ...
    async updateRail(id, dto) {
        ...
        if (dto.password)
            data.passwordRef = await this.vault.tokenize(dto.password); // re-set secret
        if (dto.ibbsId !== undefined)
            data.ibbsIdRef = dto.ibbsId ? await this.vault.tokenize(dto.ibbsId) : null;
    ...
    async testRail(id) {
        ...
        try {
            ...
            if (msg === '__timeout__')
                return { ok: false, outcome: 'unreachable', note: 'Timed out after 10s.' };
    ...
    listPermissions() {
        return permissions_catalog_1.PERMISSION_CATALOG;
    }
    ...
    validateRole(scope, permissions) {
        if (!permissions_catalog_1.ROLE_SCOPES.includes(scope))
            throw new common_1.BadRequestException(`scope must be one of ${permissions_catalog_1.ROLE_SCOPES.join(', ')}`);
        const bad = permissions.filter((p) => !permissions_catalog_1.VALID_PERMISSIONS.has(p));
        if (bad.length)
            throw new common_1.BadRequestException(`Unknown permission(s): ${bad.join(', ')}`);
    }
    ...
    async createRole(dto) {
        ...
        try {
            const role = await this.prisma.role.create({
                data: { tenantId: 't-platform', name, scope: dto.scope, permissions: dto.permissions, isSystem: false },
            });
            return { id: role.id, name: role.name, scope: role.scope, permissions: role.permissions, isSystem: role.isSystem };
        }
        catch (e) {
            if (e?.code === 'P2002')
                throw new common_1.ConflictException(`A role named '${name}' already exists.`);
            throw e;
        }
    }
    ...
    async updateRole(id, dto) {
        ...
        if (role.isSystem)
            throw new common_1.BadRequestException('System roles cannot be edited.');
        const scope = dto.scope ?? role.scope;
        const permissions = dto.permissions ?? role.permissions;
        this.validateRole(scope, permissions);
        const updated = await this.prisma.role.update({ where: { id }, data: { scope: scope, permissions } });
    ...
    async deleteRole(id) {
        ...
        if (inUse > 0)
            throw new common_1.ConflictException(`Role is assigned to ${inUse} member(s); reassign them first.`);
     ...
     */
    async importAllotments(ipoSymbol, csv, recordOne) {
        const ipo = await this.prisma.ipo.findUnique({ where: { symbol: ipoSymbol.toUpperCase() } });
        if (!ipo)
            throw new common_1.NotFoundException(`IPO '${ipoSymbol}' not found`);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!panRaw || !/^[A-Za-z]{5}[0-9]{4}[A-Za-z]$/.test(panRaw)) {
                errors.push(`${lineNo}: invalid PAN`);
                continue;
            }
            const lots = Number(lotsRaw);
            if (!Number.isInteger(lots) || lots < 0) {
                errors.push(`${lineNo}: invalid lots '${lotsRaw}'`);
                continue;
            }
            const panHash = this.vault.hash(panRaw);
            const app = await tenant_context_1.tenantContext.runUnscoped(async () => this.prisma.application.findFirst({
                where: { ipoId: ipo.id, profile: { panHash }, status: { notIn: ['draft', 'failed', 'rejected'] } },
                select: { id: true },
            }));
            if (!app) {
                notFound.push(`${lineNo}: ${this.vault.mask(panRaw)}`);
                continue;
            }
            try {
    ...
    async updateMember(slug, membershipId, patch) {
        const tenant = await this.tenantBySlug(slug);
        const ids = await this.subtreeIds(tenant.id);
        const existing = await this.prisma.membership.findUnique({ where: { id: membershipId } });
        if (!existing || !ids.includes(existing.tenantId))
            throw new common_1.NotFoundException('Membership not found in this scope');
        const data = {};
        if (patch.status)
        ...
        if (patch.roleName) {
            ...
            if (!role)
                throw new common_1.NotFoundException(`Role '${patch.roleName}' not found`);
            data.roleId = role.id;
        }
