...
export default async function IpoDetailPage({ params }: { params: { symbol: string } }) {
  const lang: Lang = 'en';
  const tr = makeT(lang);
  const q = '';
  const ipo = await getIpoDetail(params.symbol);
  if (!ipo) return <p className="muted">{tr('detail.notFound')} <a className="linklike" href={`/${q}`}>← All IPOs</a></p>;
  ...
  return (
    <article className="fade-up">
      <a href={`/${q}`} className="back-link">← All IPOs</a>
