        ...
        <>
          ...
          <div className="preanc-list">
            {parsed.investors.map((i, n) => (
              <div key={`${i.name}-${i.shares}-${n}`} className="preanc-row same" style={{ gridTemplateColumns: '30px 1fr 130px 60px' }}>
                <span className="mono" style={{ fontSize: 12, color: 'var(--text-faint)' }}>{n + 1}</span>
                <div className="preanc-cell"><div className="preanc-value">{i.name}</div></div>
          ...
          <p className="preanc-sub" style={{ marginTop: 12 }}>
            Corrections happen on the form. Extractor artefacts in fund names
            (odd characters, wrapped lines) are worth a look before you Save.
          </p>
        </>
      )}
