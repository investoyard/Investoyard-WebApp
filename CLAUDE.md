...
## Status (as of 2026-08-26) — LIVE, deployed on this Windows server
...
- **CSS: check for a name collision before adding a class.** `globals.css` is one big global sheet and it has bitten twice: a lane cell named `.sh` inherited a section utility's 76px vertical padding and blew the banner from 302px to 495px; `.nav-link` was defined for BOTH the front-site header and the admin sidebar, and the admin copy (later in the file) dragged `display:flex; gap:11px; width:100%` onto the site nav — that gap was the hole between "IPOs" and its count badge. Admin nav rules are now scoped to `.admin-side`. The build can't catch either; **measure the live DOM**.
...
- **Untested**: registrar upload end-to-end (ALTREJ.DBF / MOLBIO XLSB + one archive→restore cycle), Excel catalog import end-to-end, partner-API end-to-end (needs a key), **partner self-onboarding end-to-end** (apply → approve → activate), **GMP entry** (both a contributor and a `gmp.submit` operator), MANIPAL blank on a live IPO, WhatsApp channel URL not yet configured, reminders/bells, first news post, mobile device walk-through.
